...
export default function NutritionScreen() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/diet/log?date=${dateStr}`);
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/diet/stats`);
      const data = await response.json();
      if (data.stats) {
        setStats(data.stats);
      }
  ...
  const fetchWaterLogs = async () => {
    try {
      const dateStr = selectedDate.toISOString().split('T')[0];
      const response = await fetch(`${API_BASE_URL}/api/water/log?date=${dateStr}`);
      const data = await response.json();
  ...
  const fetchWaterStats = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/water/stats`);
      const data = await response.json();
      if (data.stats) {
    ...
    <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()}>
          <Ionicons name="arrow-back" size={24} color="#2D2E49" />
        </TouchableOpacity>
      ...
      <View style={styles.sectionContainer}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.progressBarBg}>
            <View style={[styles.progressBarFill, { width: `${Math.min((totals.cal / 2500) * 100, 100)}%`, backgroundColor: '#6C63FF' }]} />
          </View>
        ...
        <View style={[styles.card, { backgroundColor: '#E3F2FD' }]}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View>
              ...
              <Text style={[styles.statsValue, { color: '#2196F3' }]}>{waterTotal} ml</Text>
            </View>
            <TouchableOpacity onPress={() => setCurrentView('HYDRATION')}>
              <Ionicons name="arrow-forward-circle" size={40} color="#2196F3" />
            </TouchableOpacity>
          </View>
        </View>
      </View>
        ...
        <View style={styles.sectionContainer}>
          ...
          {stats.slice().reverse().map((item, idx) => (
            <View key={idx} style={styles.logItem}>
              <Ionicons name="calendar-outline" size={24} color="#6C63FF" style={{ marginRight: 15 }} />
              <View style={styles.logInfo}>
  ...
  const renderHydration = () => {
    return (
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        {/* Daily Summary Card */}
        <View style={styles.sectionContainer}>
          <View style={[styles.card, { backgroundColor: '#E3F2FD', alignItems: 'center', paddingVertical: 30 }]}>
            <Ionicons name="water" size={64} color="#2196F3" />
            <Text style={[styles.statsValue, { color: '#2196F3', marginTop: 10 }]}>{waterTotal} <Text style={{ fontSize: 18 }}>ml</Text></Text>
        ...
        <View style={styles.sectionContainer}>
          ...
          <View style={[styles.card, { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }]}>
            <TouchableOpacity onPress={() => logWater(250, true)} style={styles.cupIconBtn}>
              <Ionicons name="beer-outline" size={32} color="#2196F3" />
              <Text style={styles.cupInfo}>1 Cup</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => logWater(500, true)} style={styles.cupIconBtn}>
              <View style={{ flexDirection: 'row' }}>
                <Ionicons name="beer-outline" size={32} color="#2196F3" />
                <Ionicons name="beer-outline" size={32} color="#2196F3" style={{ marginLeft: -15 }} />
              </View>
        ...
        <View style={styles.sectionContainer}>
          ...
          <View style={[styles.card, { padding: 25, backgroundColor: '#E3F2FD' }]}>
            <View style={styles.chartContainer}>
              {renderChartGrid()}
...
const NavItem = ({ icon, label, active, onPress }: any) => (
  <TouchableOpacity style={styles.navItem} onPress={onPress}>
    <Ionicons name={icon} size={24} color={active ? '#6C63FF' : '#BDBDBD'} />
...
const styles = StyleSheet.create({
  ...
  chartBarFillWater: {
    width: '100%',
    backgroundColor: '#4facfe', // New color for water
    borderRadius: 6,
    minHeight: 4,
  },
  gridLineContainer: {
