...
export default function ForgotPassword() {
  ...
  const handleSendOTP = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://192.168.8.184:5000/api/users/forgot-password", {
        method: 'POST',
  ...
  const handleResetPassword = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://192.168.8.184:5000/api/users/reset-password", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
